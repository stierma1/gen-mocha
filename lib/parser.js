...
var objectIdFiles = glob.sync(__dirname + "/patterns/identifiers/objects/**/*.js");
var objectIdentifiers = [];
for(var i in objectIdFiles){
  objectIdentifiers.push(require(objectIdFiles[i]));
}
...
function classify(data){
  ...
  for(var i in data.ast.body){
    ...
    var objectUnion = junify.unify(expr, objectPattern);
    if(objectUnion){
      var hashes = objectUnion.hashes;
      var obj = [];
      for(var j in hashes){
        for(var k in objectIdentifiers){
          var objFuncUnion = junify.unify(hashes[j], objectIdentifiers[k]);
          if(objFuncUnion){
            obj.push(objFuncUnion);
          }
        }
      }
...
function transform(data){
  ...
  for(var i in data.objects){
    data.objects[i].map(function(val){
      data.statics = data.statics.concat(val);
    });
  }
